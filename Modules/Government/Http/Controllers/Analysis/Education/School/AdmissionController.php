...
use Modules\Government\Charts\Education\School\AdmissionChart;
...
class AdmissionController extends Controller
{
    ...
    {
        return view('government::Analysis.Education.School.Admission.index',['years'=>Year::all()]);
    }
     ...
     * @return Response
     */
    public function showResult(AdmissionChart $admission)
    {
        return view('government::Analysis.Education.School.Admission.result',['admission'=>$admission->createChart()]);
    }
    /**
        ...
        if($request->town_id){
            ...
            if($flag == 'year'){
                foreach ($town->areas as $area) {
                    $count = 0;
                    foreach (AreaSchoolReportCollation::where(['area_id'=>$area->id,'year_id'=>$request->year_id])->get() as $area_school_admission) {
                        $count = $area_school_admission->yearly_admission + $count;
                    }
                ...
                foreach ($town->areas as $area) {
                    $count = 0;
                    foreach (AreaSchoolReportCollation::where(['area_id'=>$area->id])->get() as $area_school_admission) {
                        $count = $area_school_admission->yearly_admission + $count;
                    }
                    $labels[] = $area->name;
                    $data_sets[] = $count; 
            ...
            if($flag == 'year'){
                foreach ($district->towns as $town) {
                    $count = 0;
                    foreach (TownSchoolReportCollation::where(['town_id'=>$town->id,'year_id'=>$request->year_id])->get() as $town_school_admission) {
                        $count = $town_school_admission->yearly_admission + $count;
                    }
                ...
                foreach ($district->towns as $town) {
                    $count = 0;
                    foreach (TownSchoolReportCollation::where(['town_id'=>$town->id])->get() as $town_school_admission) {
                        $count = $town_school_admission->yearly_admission + $count;
                    }
                    $labels[] = $town->name;
                    $data_sets[] = $count; 
            ...
            if($flag == 'year'){
                foreach ($lga->districts as $district) {
                    $count = 0;
                    foreach (DistrictSchoolReportCollation::where(['district_id'=>$district->id,'year_id'=>$request->year_id])->get() as $district_school_admission) {

                        $count = $district_school_admission->yearly_admission + $count;
                    }
                ...
                foreach ($lga->districts as $district) {
                    $count = 0;
                    foreach (DistrictSchoolReportCollation::where(['district_id'=>$district->id])->get() as $district_school_admission) {
        ...
        */
        if(government()->state){
            ...
            foreach (government()->state->lgas as $lga) {
                $count = 0;
                foreach ($lga->lgaSchoolReportCollations as $school_admission) {
                    $count = $school_admission->yearly_admission + $count;
                }
                $data_sets[] = $count;
                $labels[] = $lga->name;
            }
            ...
            foreach (government()->lga->districts as $district) {
                $count = 0;
                foreach ($district->districtSchoolReportCollations as $school_admission) {
                    $count = $school_admission->yearly_admission + $count;
                }
                $data_sets[] = $count;
                $labels[] = $district->name;
            ...
            foreach (government()->district->towns as $town) {
                $count = 0;
                foreach ($town->townSchoolReportCollations as $school_admission) {
                    $count = $school_admission->yearly_admission + $count;
                }
                $data_sets[] = $count;
                $labels[] = $town->name;
        ...
        return redirect()->route('government.analysis.education.school.admission.result');
    }
}
